...
select app in bazarr lidarr prowlarr radarr readarr sonarr quit; do
    ...
    bazarr)
        ...
        app_prereq="curl sqlite3 python3-dev python3-pip python3-distutils unzip"	# Required packages
...
# get arch
	DLURL="https://github.com/morpheus65535/bazarr/releases/latest/download/bazarr.zip"
	echo ""
	echo "Downloading..."
...
Group=$app_guid
UMask=$app_umask
ExecStart=/usr/bin/mono --debug $bindir/$app_bin.exe -nobrowser -data=$datadir
Type=simple
TimeoutStopSec=20
KillMode=process
Restart=on-failure
